...
useEffect(() =>  { EFFECT });
`);
const buildUseCallbackHook = template_1.default(`
...
function statefulToStateless(component) {
    ...
    let stateHooksPresent, nonLifeycleMethodsPresent = false;
    ...
    const namedArrowFunction = ({ name, params = [], propType = null, paramDefaults = [], body = [], arrowFunctionCreator = arrowFunction }) => {
        const identifier = t.identifier(name);
        addPropTSAnnotationIfNeeded(propType, identifier);
        return t.variableDeclaration("const", [
            t.variableDeclarator(identifier, arrowFunctionCreator(params, paramDefaults, body))
        ]);
    };
        ...
        if (name !== "render") {
            if (settings_1.isHooksForFunctionalComponentsExperimentOn()) {
                functionBody.push(namedArrowFunction({
                    name,
                    body,
                    arrowFunctionCreator: (params, paramDefaults, arrowBody) => buildUseCallbackHook({
...
function arrowFunction(params, paramDefaults, body) {
...
function resolveTypeAnnotation(propType) {
    ...
    const componentTypeAnnotation = settings_1.isHooksForFunctionalComponentsExperimentOn()
        ? "FC"
        : "SFC";
...
function statefulToStatelessComponent() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const answer = settings_1.shouldShowConversionWarning() ? yield editor_1.showInformationMessage("WARNING! All lifecycle methods and react instance methods would be removed. Are you sure you want to continue?", ["Yes", "No"]) : "Yes";
            if (answer === "Yes") {
                ...
                const { stateHooksPresent, nonLifeycleMethodsPresent } = selectionProccessingResult.metadata;
                const usedHooks = [
